...
| **Conseiller** | Le membre du personnel CPI responsable d'un dossier. DEUX colonnes, tenues ensemble par `AttributionConseiller` : `clients.conseiller_id` (relation, lue par le cloisonnement) et `clients.conseiller` (texte, lu par TOUT l'affichage client — tableau de bord, chantier, « Ma demande », profil). Écrire l'une sans l'autre laisse le client voir « Non assigné » malgré un accès déjà restreint à un agent précis. Cloisonnement STRICT depuis STEP 4 : un agent ne voit QUE son portefeuille — un dossier non attribué n'est plus visible d'aucun agent, seul le super-admin l'atteint (`?non_attribues=1`, `POST .../attribuer-conseiller`). |
| **Attribution automatique** | Déclenchée par la validation d'un compte (`CompteValidationController::valider`) : le dossier part à l'agent-cpi dont le portefeuille ACTIF (avant `ParcoursDossier::SIGNATURE`, hors dossiers supprimés) est le plus léger. Aucun agent-cpi disponible ⇒ le compte est validé quand même, le dossier reste sans conseiller. Voir `App\Support\AttributionConseiller`. |
| **Réattribution manuelle** | `PUT /staff/clients/{client}/conseiller`, réservée au super-admin (`manage-staff`) — l'administrateur CHOISIT explicitement le nouvel agent, à la différence de l'attribution automatique qui élit le moins chargé. Déplace un dossier déjà suivi (départ d'un agent, réorganisation) ou attribue un dossier resté sans conseiller. Les DEUX agents (ancien et nouveau) sont notifiés côté personnel, le client reçoit la même notification qu'une première attribution. Voir `AttributionConseiller::assignerManuellement()`. |
